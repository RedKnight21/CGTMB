/*=========================================================================
  ...
  Module:    sssIntersectionPolyDataCheck.cxx
...
//------------------------------------------------------------------------------
class sssIntersectionPolyDataCheck::Impl
{
public:
  Impl();
  virtual ~Impl();
...
public:
  ...
  sssIntersectionPolyDataCheck* ParentFilter;
...
//------------------------------------------------------------------------------
sssIntersectionPolyDataCheck::Impl::Impl()
  : OBBTree1(nullptr)
...
//------------------------------------------------------------------------------
int sssIntersectionPolyDataCheck::Impl ::FindTriangleIntersections(
  vtkOBBNode* node0, vtkOBBNode* node1, vtkMatrix4x4* transform, void* arg)
{
  sssIntersectionPolyDataCheck::Impl* info =
    reinterpret_cast<sssIntersectionPolyDataCheck::Impl*>(arg);
      ...
      if (obbTree1->TriangleIntersectsNode(node1, triPts0[0], triPts0[1], triPts0[2], transform))
      {
        ...
        for (vtkIdType id1 = 0; id1 < numCells1; id1++)
        {
          ...
          if (type1 == VTK_TRIANGLE)
          {
            ...
            double surfaceid[2];
            int intersects = sssIntersectionPolyDataCheck::TriangleTriangleIntersection(triPts0[0],
...
//------------------------------------------------------------------------------
int sssIntersectionPolyDataCheck::Impl ::SplitMesh(
  int inputIndex, vtkPolyData* output, vtkPolyData* intersectionLines)
{
  ...
  if (input->GetPolys()->GetNumberOfCells() > 0)
  {
    ...
    for (cells->InitTraversal(); cells->GetNextCell(nptsX, pts); cellIdX++)
    {
      if (nptsX != 3)
      {
        vtkGenericWarningMacro(<< "sssIntersectionPolyDataCheck only works"
...
vtkCellArray* sssIntersectionPolyDataCheck::Impl ::SplitCell(vtkPolyData* input, vtkIdType cellId,
...
//------------------------------------------------------------------------------
int sssIntersectionPolyDataCheck::Impl ::AddToPointEdgeMap(int index, vtkIdType ptId, double x[3],
...
void sssIntersectionPolyDataCheck::Impl::AddToNewCellMap(
  int inputIndex, int interPtCount, int interPts[3], vtkPolyData* interLines, int numCurrCells)
{
  vtkIdList** cellIds;
...
int sssIntersectionPolyDataCheck::Impl ::GetLoops(vtkPolyData* pd, std::vector<simPolygon>* loops)
{
...
int sssIntersectionPolyDataCheck::Impl ::GetSingleLoop(vtkPolyData* pd, simPolygon* loop,
...
int sssIntersectionPolyDataCheck::Impl ::FollowLoopOrientation(vtkPolyData* pd, simPolygon* loop,
...
void sssIntersectionPolyDataCheck::Impl ::SetLoopOrientation(vtkPolyData* pd, simPolygon* loop,
  vtkIdType* nextCell, vtkIdType nextPt, vtkIdType prevPt, vtkIdList* pointCells)
{
...
int sssIntersectionPolyDataCheck::Impl::GetLoopOrientation(
...
void sssIntersectionPolyDataCheck::Impl ::Orient(
  vtkPolyData* pd, vtkTransform* transform, vtkPolyData* boundary, vtkPolygon* boundarypoly)
{
...
int sssIntersectionPolyDataCheck::Impl::CheckLine(
...
int sssIntersectionPolyDataCheck::Impl::GetTransform(vtkTransform* transform, vtkPoints* points)
{
...
//------------------------------------------------------------------------------
vtkStandardNewMacro(sssIntersectionPolyDataCheck);

//------------------------------------------------------------------------------
sssIntersectionPolyDataCheck::sssIntersectionPolyDataCheck()
  : SplitFirstOutput(1)
  , SplitSecondOutput(1)
{
...
//------------------------------------------------------------------------------
sssIntersectionPolyDataCheck::~sssIntersectionPolyDataCheck() = default;

//------------------------------------------------------------------------------
void sssIntersectionPolyDataCheck::PrintSelf(ostream& os, vtkIndent indent)
{
...
//------------------------------------------------------------------------------
int sssIntersectionPolyDataCheck::TriangleTriangleIntersection(double p1[3], double q1[3],
...
void sssIntersectionPolyDataCheck::CleanAndCheckSurface(
...
void sssIntersectionPolyDataCheck::CleanAndCheckInput(vtkPolyData* pd, double tolerance)
{
  vtkSmartPointer<vtkCleanPolyData> cleaner = vtkSmartPointer<vtkCleanPolyData>::New();
  vtkSmartPointer<vtkTriangleFilter> triangulator = vtkSmartPointer<vtkTriangleFilter>::New();
...
//------------------------------------------------------------------------------
int sssIntersectionPolyDataCheck::RequestData(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* outputVector)
{
  ...
  sssIntersectionPolyDataCheck::Impl* impl = new sssIntersectionPolyDataCheck::Impl();
  ...
  obbTree0->IntersectWithOBBTree(
    obbTree1, nullptr, sssIntersectionPolyDataCheck::Impl::FindTriangleIntersections, impl);
...
//	printf(" this->NumberOfIntersectionPoints: %d\n", this->NumberOfIntersectionPoints);
//    printf(" this->NumberOfIntersectionLines: %d\n", this->NumberOfIntersectionLines);
  ...
  if (this->NumberOfIntersectionPoints == 0 || this->NumberOfIntersectionLines == 0)
  {
    ...
    delete impl;
//    printf("sssIntersectionPolyDataCheck:: return 0.\n");
    return 0;
  }
  ...
  delete impl;
//  printf("sssIntersectionPolyDataCheck:: return 1.\n");
  return 1;
...
//  printf("next is split second output\n");
...
//------------------------------------------------------------------------------
int sssIntersectionPolyDataCheck::FillInputPortInformation(int port, vtkInformation* info)
{
